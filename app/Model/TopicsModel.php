...
class TopicsModel extends Model
{
	...
	*/
	public function search($attributes){
		...
		$sql_req = 'SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, closed FROM f_topics ';

		$i = 0;
		foreach($attributes as $q) {		
			if($i == 0){
				$sql_req .= 'WHERE title ';
			} else {
				$sql_req .= ' AND title ';
		...
		$search = $this->my_sql->prepare($sql_req, $attributes, null, true);
		return $search;
	}
	...
	*/
	public function selectBylimit($limit1 = null, $limit2 = null){
		if($limit1 !== null && $limit2 !== null){
			$topics = $this->my_sql->query('SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, closed FROM f_topics ORDER BY creation_date DESC LIMIT '.$limit1.', '.$limit2);
		} elseif($limit1 !== null && $limit2 === null) {
			$topics = $this->my_sql->query('SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, closed FROM f_topics ORDER BY creation_date DESC LIMIT '.$limit1);
		}
		return $topics;
	}
	...
	*/
	public function select($attributes = null, $where = null, $all = null){
		if($where !== null && $all === null){
			$topics = $this->my_sql->prepare('SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, user_notif, closed FROM f_topics WHERE ' . $where .'= ?', $attributes, true);
		} elseif($where !== null && $all !== null){
			$topics = $this->my_sql->prepare('SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, user_notif, closed FROM f_topics WHERE ' . $where .'= ?', $attributes, null, true);
		} else {
			$topics = $this->my_sql->query('SELECT id, user_id, title, content, DATE_FORMAT(creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, resolved, closed FROM f_topics ORDER BY creation_date DESC');
		}
	...
	public function selectByCategory($attributes, $where, $limit1 = null, $limit2 = null){
		if($limit1 !== null && $limit2 !== null){
			$topics = $this->my_sql->prepare('
				SELECT f_topics.id, f_topics.user_id, f_topics.title, f_topics.content, DATE_FORMAT(f_topics.creation_date, "%d/%m/%Y à %Hh%imin%ss") as date_topic, DATE_FORMAT(f_topics.edit_date, "%d/%m/%Y à %Hh%imin%ss") as t_edit_date, f_topics.resolved, f_topics.closed FROM f_topics 
	...
	*/
	public function close($attributes){
		$close = $this->my_sql->prepare('UPDATE f_topics SET closed = 1 WHERE id = ?', $attributes);
		return $close;
	}
}